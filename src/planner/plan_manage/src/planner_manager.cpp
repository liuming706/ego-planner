                                      ...
                                      bool flag_polyInit, bool flag_randomPolyTraj)
{
    ...
    do {
        ...
        if (flag_first_call || flag_polyInit ||
            flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/)  // Initial path generated from a min-snap traj by order.
        {
            ...
            double time = pow(pp_.max_vel_, 2) / pp_.max_acc_ > dist
                              ...
                              : (dist - pow(pp_.max_vel_, 2) / pp_.max_acc_) /
                                        pp_.max_vel_ +
        ...
        } else  // Initial path generated from previous trajectory.
        {
            ...
            double cps_dist = pp_.ctrl_pt_dist *
                              1.5;  // cps_dist will be divided by 1.5 in the next
            size_t id = 0;
            do {
                cps_dist /= 1.5;
                point_set.clear();
    ...
    if (!flag_step_2_success) {
        printf(
            "\033[34mThis refined trajectory hits obstacles. It doesn't matter "
            "if appeares occasionally. But if continously appearing, Increase "
            "parameter \"lambda_fitness\".\n\033[0m");
        continous_failures_count_++;
        return false;
    }
                                       ...
                                       Eigen::MatrixXd &optimal_control_points)
{
    ...
    bool success = bspline_optimizer_rebound_->BsplineOptimizeTrajRefine(
