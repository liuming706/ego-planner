...
void EGOReplanFSM::planGlobalTrajbyGivenWps()
{
    ...
    for (size_t i = 0; i < (size_t)waypoint_num_; i++) {
        visualization_->displayGoalPoint(
            wps[i], Eigen::Vector4d(0, 0.5, 0.5, 1), 0.3, i);
    ...
    if (success) {
        ...
        for (int i = 0; i < i_end; i++) {
            gloabl_traj[i] = planner_manager_->global_data_.global_traj_.evaluate(
                i * step_size_t);
        }
...
void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
{
    ...
    static string state_str[7] = {"INIT",         "WAIT_TARGET",
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
{
    ...
    for (double t = t_cur; t < info->duration_; t += time_step) {
        if (t_cur < t_2_3 &&
            t >= t_2_3)  // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is considered valid and will get checked.
            break;
        ...
        if (map->getInflateOccupancy(info->position_traj_.evaluateDeBoorT(t))) {
            if (planFromCurrentTraj())  // Make a chance
            {
                ...
                {
                    ROS_WARN(
                        "Suddenly discovered obstacles. emergency stop! "
                        "time=%f",
                        t - t_cur);
