...
GradientDescentOptimizer::RESULT GradientDescentOptimizer::optimize(
    Eigen::VectorXd &x_init_optimal, double &opt_f)
{
    if (min_grad_ < 1e-10) {
    ...
    double alpha0 = max_grad < MAX_MOVEMENT_AT_FIRST_ITERATION
                        ? 1.0
                        : (MAX_MOVEMENT_AT_FIRST_ITERATION / max_grad);
    ...
    while (++iter <= iter_limit_ && invoke_count <= invoke_limit_) {
        ...
        if (iter % 2)  // to aviod copying operations
        {
            do {
                ...
                if (force_return) return RETURN_BY_ORDER;
                alpha *= 0.5;
            } while (cost_k > cost_kp1 - 1e-4 * alpha * grad_kp1.transpose() *
                                             grad_kp1);  // Armijo condition
            ...
            do {
                ...
                if (force_return) return RETURN_BY_ORDER;
                alpha *= 0.5;
            } while (cost_kp1 > cost_k - 1e-4 * alpha * grad_k.transpose() *
                                             grad_k);  // Armijo condition
