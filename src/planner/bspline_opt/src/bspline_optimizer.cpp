    ...
    Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
{
    ...
    constexpr double MINIMUM_PERCENT =
        0.0;  // Each segment is guaranteed to have sufficient points to
    ...
    for (size_t i = 0; i < segment_ids.size(); i++) {
        ...
        // cout << "i = " << i << " first = " << segment_ids[i].first << "
        // second = " << segment_ids[i].second << endl;
        ...
        // final_segment_ids[i].first << " second = " << final_segment_ids[i].second
        // << endl;
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++) {
        // step 1
        for (int j = final_segment_ids[i].first;
             j <= final_segment_ids[i].second; ++j)
        ...
        /* Corner case: the segment length is too short. Here the control points
         * may outside the A* path, leading to opposite gradient direction. So I
        ...
        if (segment_ids[i].second - segment_ids[i].first == 1) {
            ...
            int Astar_id = a_star_pathes[i].size() / 2,
                last_Astar_id;  // Let "Astar_id =
                                // id_of_the_most_far_away_Astar_point" will be
                                // better, but it needs more computation
            double val = (a_star_pathes[i][Astar_id] - middle_point).dot(ctrl_pts_law),
                   last_val = val;
            while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size()) {
                last_Astar_id = Astar_id;
                                               ...
                                               int iter_num, double smoothness_cost)
{
    ...
    for (auto i = order_; i < end_idx; ++i) {
        for (size_t j = 0; j < cps_.direction[i].size(); ++j) {
            double dist =
                (cps_.points.col(i) - cps_.base_point[i][j]).dot(cps_.direction[i][j]);
        ...
        for (int j = 0; j < 3; j++) {
            if (vi(j) > max_vel_ + demarcation) {
                ...
                cost += (ar * diff * diff + br * diff + cr) *
                        ts_inv3;  // multiply ts_inv3 to make vel and acc has
                                  // similar magnitude
        ...
        for (int j = 0; j < 3; j++) {
            if (vi(j) > max_vel_) {
                ...
                cost += pow(vi(j) - max_vel_, 2) *
                        ts_inv2;  // multiply ts_inv3 to make vel and acc has
                                  // similar magnitude
...
bool BsplineOptimizer::check_collision_and_rebound(void)
{
    ...
    for (int i = order_ - 1; i <= i_end; ++i) {
            ...
            for (size_t k = 0; k < cps_.direction[i].size(); ++k) {
                ...
                if ((cps_.points.col(i) - cps_.base_point[i][k])
                    ...
                    1 * grid_map_->getResolution())  // current point is outside all the
                                                     // collision_points.
                {
                    occ = false;  // Not really takes effect, just for better
                                  // hunman understanding.
                    break;
                }
            }
        }
        ...
        if (occ) {
            ...
            {
                ROS_WARN(
                    "WARN! terminal point of the current trajectory is in "
    ...
    if (flag_new_obs_valid) {
        ...
        for (size_t i = 0; i < segment_ids.size(); ++i) {
            ...
            for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
                ...
                int Astar_id = a_star_pathes[i].size() / 2,
                    last_Astar_id;  // Let "Astar_id =
                                    // id_of_the_most_far_away_Astar_point" will
                                    // be better, but it needs more computation
                ...
                if (got_intersection_id >= 0) {
                    cps_.flag_temp[j] = true;
                    double length =
                        (intersection_point - cps_.points.col(j)).norm();
                    if (length > 1e-5) {
                        for (double a = length; a >= 0.0;
                             a -= grid_map_->getResolution()) {
...
bool BsplineOptimizer::BsplineOptimizeTrajRebound(Eigen::MatrixXd &optimal_points,
                                                  double ts)
{
...
bool BsplineOptimizer::rebound_optimize()
{
    ...
    do {
        ...
        if (result == lbfgs::LBFGS_CONVERGENCE ||
                               ...
                               // whole trajectory.
            {
                ...
                if (flag_occ) {
                    ...
                    if (t <= bspline_interval_)  // First 3 control points in obstacles!
                    {
                        ...
                        ROS_WARN(
                            "First 3 control points in obstacles! return "
                            "false, t=%f",
                            t);
                        return false;
                    }
            ...
            if (!flag_occ) {
                printf(
                    "\033[32miter(+1)=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost="
                    "%5.3f\n\033[0m",
            ...
            } else  // restart
            {
                ...
                printf(
                    "\033[32miter(+1)=%d,time(ms)=%5.3f,keep "
                    "optimizing\n\033[0m",
                    iter_num_, time_ms);
            }
            ...
            rebound_times++;
            cout << "iter=" << iter_num_ << ",time(ms)=" << time_ms
                 << ",rebound." << endl;
        } else {
            ROS_WARN("Solver error. Return = %d, %s. Skip this planning.",
...
bool BsplineOptimizer::refine_optimize()
{
    ...
    do {
        ...
        double t_step =
            (tmp - tm) /
            ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() /
             grid_map_->getResolution());  // Step size is defined as the maximum size
                                          ...
                                          double &f_combine, const int n)
{
    ...
    // printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility,
    // f_combine);
