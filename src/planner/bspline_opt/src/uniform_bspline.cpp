...
bool UniformBspline::checkFeasibility(double &ratio, bool show)
{
    ...
    for (int i = 0; i < P.cols() - 1; ++i) {
        ...
        if (fabs(vel(0)) > enlarged_vel_lim ||
            fabs(vel(1)) > enlarged_vel_lim || fabs(vel(2)) > enlarged_vel_lim) {
            if (show)
                cout << "[Check]: Infeasible vel " << i << " :"
                     << vel.transpose() << endl;
            fea = false;
    ...
    for (int i = 0; i < P.cols() - 2; ++i) {
        ...
        if (fabs(acc(0)) > enlarged_acc_lim ||
            fabs(acc(1)) > enlarged_acc_lim || fabs(acc(2)) > enlarged_acc_lim) {
            if (show)
                cout << "[Check]: Infeasible acc " << i << " :"
                     << acc.transpose() << endl;
            fea = false;
