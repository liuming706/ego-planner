...
public:
    ...
    FG_eval(Eigen::MatrixXd desired_state) { desired_state_ = desired_state; }
    ...
    void operator()(ADvector &fg, const ADvector &vars)
    {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (psi_des < -M_PI) psi_des += 2 * M_PI;
            fg[0] += psi_weight *
                     CppAD::pow(vars[psi_start + i] - psi_des + M_PI / 2, 2);
        }
        for (int i = 0; i < N - 1; ++i) {
        ...
        for (int i = 1; i < N; ++i) {
            ...
            fg[1 + x_start + i] =
                x_1 -
                (x_0 + (u_0 * CppAD::cos(psi_0) - v_0 * CppAD::sin(psi_0)) * dt);
            fg[1 + y_start + i] =
                y_1 -
                (y_0 + (u_0 * CppAD::sin(psi_0) + v_0 * CppAD::cos(psi_0)) * dt);
...
vector<double> Mpc::solve(Eigen::Vector3d state, Eigen::MatrixXd desired_state)
{
    ...
    cout << "u:" << solution.x[u_start] << " "
         << "v:" << solution.x[v_start] << endl;
    return solved;
}