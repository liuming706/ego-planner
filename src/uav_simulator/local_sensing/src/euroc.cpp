                         ...
                         const geometry_msgs::TransformStampedConstPtr &pose_input)
{
    // time diff
    double time_diff =
        fabs(image_input->header.stamp.toSec() - pose_input->header.stamp.toSec()) *
        1000.0;
...
int main(int argc, char **argv)
{
    ...
    if (nh.hasParam("cam_k1") && nh.hasParam("cam_k2") &&
        nh.hasParam("cam_r1") && nh.hasParam("cam_r2")) {
        float k1, k2, r1, r2;
        ...
        cv::initUndistortRectifyMap(cv_K, cv_D, cv::Mat_<double>::eye(3, 3),
                                    cv_K, cv::Size(width, height), CV_16SC2,
    ...
    vicon2body << 0.33638, -0.01749, 0.94156, 0.06901, -0.02078, -0.99972,
        -0.01114, -0.02781, 0.94150, -0.01582, -0.33665, -0.12395, 0.0, 0.0,
        0.0, 1.0;
