...
public:
    ...
    inline void SetOccupancyValue(int mz, int value)
    {
        ...
        {
            for (list<OccupancyGrid>::iterator k = grids.begin();
                 k != grids.end(); k++) {
...
public:
    Map3D()
    {
        ...
        logOddOccupiedThr = log(1.0 / (1.0 - PROB_OCCUPIED_THRESHOLD) - 1.0) *
                    ...
                    for (unsigned int k = 0; k < grids.size(); k++) {
                        if ((grids[k].mass / (grids[k].upper - grids[k].lower + 1) >
                            ...
                            (grids[k].mass / (grids[k].upper - grids[k].lower + 1) <
                                 logOddFreeThr &&
                             type == FREE)) {
                            for (int mz = grids[k].lower; mz <= grids[k].upper;
                                 mz++) {
                                double x, y, z;
...
private:
    ...
    inline void MapFrameToWorldFrame(int mx, int my, int mz, double &x,
                                     double &y, double &z)
    {
