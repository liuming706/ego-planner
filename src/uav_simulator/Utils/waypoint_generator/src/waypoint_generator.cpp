...
void load_seg(ros::NodeHandle &nh, int segid, const ros::Time &time_base)
{
    ...
    for (size_t k = 0; k < ptx.size(); ++k) {
        ...
        Eigen::Vector2d rdp;
        rdp.x() = std::cos(-baseyaw - yaw) * dp.x() +
                  std::sin(-baseyaw - yaw) * dp.y();
        rdp.y() = -std::sin(-baseyaw - yaw) * dp.x() +
                  std::cos(-baseyaw - yaw) * dp.y();
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    if (waypoint_type == string("circle")) {
        ...
        if (msg->pose.position.z > -0.1) {
            ...
            ROS_WARN(
                "[waypoint_generator] invalid goal in manual-lonely-waypoint "
                "mode.");
        }
    } else {
