...
int GameLikeInput::processMouseEvent(rviz::ViewportMouseEvent &event)
{
    ...
    if (selecting_) {
        ...
        sel_manager->highlight(event.viewport, sel_start_x_, sel_start_y_,
                               event.x, event.y);

        if (event.leftUp()) {
            rviz::SelectionManager::SelectType type =
            ...
            for (auto it = selection.begin(); it != selection.end();) {
                rviz::Picked p = it->second;
                rviz::SelectionHandler *sh = sel_manager->getHandler(p.handle);
                void (rviz::SelectionHandler::*vptr)(
        ...
        if (event.rightUp()) {
            ...
            if (event.shift()) {
                ...
                arrow_ = new rviz::Arrow(scene_manager_, NULL, 2.0f, 0.2f, 0.5f,
                                         0.35f);
                arrow_->setColor(0.3f, 0.35f, 0.9f, 1.0f);
                arrow_->getSceneNode()->setVisible(false);
            } else {
