...
public:
    ...
    controlled_step_result try_step(System system, const StateIn &in,
                                    ...
                                    DerivOut &dxdt_new, time_type &dt)
    {
        BOOST_USING_STD_MIN();
        BOOST_USING_STD_MAX();
        using std::pow;
                ...
                if ((k == m_current_k_opt - 1) ||
                    ...
                    if (error < 1.0) {
                        ...
                        if ((work[k] < KFAC2 * work[k - 1]) ||
                            ...
                            new_h = h_opt[k] *
                                    static_cast<value_type>(m_cost[k + 1]) /
    ...
    void initialize(const StateType &x0, const time_type &t0, const time_type &dt0)
    {
        m_resizer.adjust_size(
            x0, detail::bind(
                    &controlled_error_bs_type::template resize_impl<StateType>,
    ...
    std::pair<time_type, time_type> do_step(System system)
    {
        ...
        size_t count = 0;
        while (res == fail) {
            res = try_step(system, get_current_state(), get_current_deriv(),
                           m_t, get_old_state(), get_old_deriv(), m_dt);
            m_first = false;
            if (count++ == max_count)
                throw std::overflow_error(
                    "bulirsch_stoer : too much iterations!");
        }
...
private:
    ...
    /* k is the order to which the result was approximated */
    {
         ...
         * interpolation coefficients the values are x(t+dt/2) and the
         * derivatives dx/dt , ... d^(2k+2) x / dt^(2k+2) at the midpoints the
         * derivatives are approximated via finite differences all values are
         * obtained from interpolation of the results from the increasing orders
         * of the midpoint calls
         */
        ...
        for (int j = 0; j <= k; j++) {
            ...
            value_type f =
                1.0;  // factor 1/2 here because our interpolation interval has length 2 !!!
        ...
        if (m_control_interpolation) {
            ...
            error = m_error_checker.error(m_algebra, x_start, dxdt_start,
                                          m_err.m_v, dt);
        }
            ...
            for (int i = m + static_cast<int>(kappa) - 2;
                ...
                if (i >= 0) {
                    m_algebra.for_each3(
                        ...
                        m_diffs[kappa][j_diffs].m_v, m_derivs[j][i].m_v,
                        typename operations_type::template scale_sum2<value_type, value_type>(
                            1.0, sign * fac *
                                     boost::math::binomial_coefficient<value_type>(
                                         kappa, c)));
                } else {
                    m_algebra.for_each3(
                        m_diffs[kappa][j_diffs].m_v,
                        m_diffs[kappa][j_diffs].m_v, dxdt,
                        typename operations_type::template scale_sum2<value_type, value_type>(
                            1.0, sign * fac));
                }
                sign *= -1;
