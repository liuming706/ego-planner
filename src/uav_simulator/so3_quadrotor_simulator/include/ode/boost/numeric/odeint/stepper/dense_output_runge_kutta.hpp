...
public:
     ...
     * ordinary differential equation. It must fulfill the Simple System
     * concept. \return Pair with start and end time of the integration step.
     */
    ...
    {
        m_stepper.calc_state(x, t, get_old_state(), m_t_old,
...
public:
    ...
    std::pair<time_type, time_type> do_step(System system)
    {
        ...
        size_t count = 0;
        do {
            res = m_stepper.try_step(system, get_current_state(),
                                     get_current_deriv(), m_t, get_old_state(),
                                     get_old_deriv(), m_dt);
            if (count++ == max_count)
