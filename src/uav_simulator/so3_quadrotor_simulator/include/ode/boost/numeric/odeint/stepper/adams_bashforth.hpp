...
#ifndef DOXYGEN_SKIP
    typedef adams_bashforth<Steps, State, Value, Deriv, Time, Algebra,
                            Operations, Resizer, InitializingStepper>
        stepper_type;
#endif
...
private:
                      ...
                      StateOut &out, time_type dt)
    {
        ...
        if (m_steps_initialized < steps - 1) {
            ...
            m_initializing_stepper.do_step(system, in, m_step_storage[0].m_v, t,
                                           out, dt);
            m_steps_initialized++;
        } else {
 ...
 * ordinary differential equation. It must fulfill the Simple System concept. \param
 * x The state of the ODE which should be solved. After calling do_step the
 * result is updated in x. \param t The value of the time, at which the step
 ...
 * state x and time t are updated to the values after Steps-1 initial steps. \param
 * system The system function to solve, hence the r.h.s. of the ordinary
 * differential equation. It must fulfill the Simple System concept. \param x
