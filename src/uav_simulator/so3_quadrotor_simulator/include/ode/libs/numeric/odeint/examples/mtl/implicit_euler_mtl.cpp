...
struct system2_mtl4 {
    void operator()(const vec_mtl4 &x, vec_mtl4 &dxdt, double t)
    {
        ...
        for (int i = 0; i < size / 5; i += 5) {
            dxdt[i] = -0.5 * x[i];
            dxdt[i + 1] = +25 * x[i + 1] * x[i + 2] -
                          740 * x[i + 3] * x[i + 3] + 4.2e-2 * x[i];
...
void testRidiculouslyMassiveArray(int size)
{
    ...
    size_t num_of_steps_ublas = integrate_const(
        booststepper(), make_pair(system1_ublas(), jacobi1_ublas()), x_ublas,
        0.0, endtime, dt);
    ...
    clog << "dt_ublas/dt_mtl4 = "
         << (double)(tend_boost - tstart_boost) / (tend_mtl4 - tstart_mtl4) << endl
         << endl;
    return;
}
