                                 ...
                                 typename Stepper::state_type &xerr)
{
    ...
    stepper.do_step(
        system, typename boost::add_reference<container_type>::type(x), 0.0,
        0.1, typename boost::add_reference<container_type>::type(xerr));
}
...
struct perform_error_stepper_test<Stepper, vector_type> {
    void operator()(void)
    {
        ...
        check_error_stepper_concept(
            stepper, constant_system_standard<vector_type, vector_type, double>,
            x, xerr);
        check_error_stepper_concept(
...
struct perform_error_stepper_test<Stepper, array_type> {
    void operator()(void)
    {
        ...
        Stepper stepper;
        check_error_stepper_concept(
            stepper, constant_system_standard<array_type, array_type, double>,
            x, xerr);
        check_error_stepper_concept(
            stepper, boost::cref(constant_system_functor_standard()), x, xerr);
        BOOST_CHECK_SMALL(fabs(x[0] - result), eps);
